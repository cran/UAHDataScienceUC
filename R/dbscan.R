...
#' @param epsilon how close two observations have to be to be considered neighbors.
...
#'
#' \enumerate{
#'  \item For each point, the neighborhood of radius \code{epsilon} is computed. If
...
#'  \code{epsilon} \tab the value of \code{epsilon} used. \cr
...
dbscan <- function(
  ...
  if (learn) {
    ...
    console.log("")
    console.log("    1. For each point, the neighborhood of radius epsilon is computed. If the neighborhood contains at least min_pts points, then the point is considered a core point. Otherwise, the point is considered an outlier.")
  ...
  for (idx in seq_len(nrow(data))) {
    if (cluster_id$of[idx] != -1)
      next
    if (expand_cluster(neighbors, cluster_id, idx, min_pts, learn, waiting))
  ...
  if (ncol(data) == 2) {
    plot(data, col = cluster_id$of + 1, pch = 20, asp = 1)
  }
