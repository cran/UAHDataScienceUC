...
#' @param distance_method the distance measure to use. Supported values are:
#'   \itemize{
...
agglomerative_clustering <- function(
  ...
  # Valid distances from user perspective
  valid_distances <- c("euclidean", "manhattan", "canberra", "chebyshev")

  # Validate distance method
  if(!distance_method %in% valid_distances) {
    stop(sprintf("Invalid distance method. Must be one of: %s",
                 paste(valid_distances, collapse = ", ")))
  }

  # Map chebyshev to maximum for proxy package
  if(distance_method == "chebyshev") {
  ...
  if (learn) {
    ...
    # Display the chosen distance formula
  ...
  d <- as.matrix(proxy::dist(data, method = distance_method, ...))
  d <- mapply(
    "[<-",
    data.frame(d),
    seq_len(nrow(data)),
  ...
  ans$dist.method <- distance_method
  ...
  if (learn) {
    ...
    # Plot dendrogram even when learn=FALSE
    plot(ans, hang = -1, main = "Hierarchical Clustering Dendrogram")
  }
